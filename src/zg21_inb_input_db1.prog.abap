*&---------------------------------------------------------------------*
*& Include ZG21_INB_INPUT_DB1 - Main Program Logic (ĐÃ FIX VÀ TỐI ƯU HÓA)
*&---------------------------------------------------------------------*
...
* 1. Kiểm tra kết quả
  IF lt_vbeln IS INITIAL.
    MESSAGE 'Không tìm thấy Inbound Delivery nào phù hợp với tiêu chí chọn để in.' TYPE 'I'.
    EXIT.
  ENDIF.
...
* 2. Chuyển đổi danh sách VBELN thành Selection-Screen Parameters (RSPARAMS)
  LOOP AT lt_vbeln INTO lv_vbeln.
    ...
    <fs_param>-kind   = 'S'.        " S = SELECT-OPTIONS
    <fs_param>-sign   = 'I'.        " I = Include
    <fs_param>-option = 'EQ'.       " EQ = Equal
    <fs_param>-low    = lv_vbeln.
  ENDLOOP.

* 3. Gọi chương trình in Smartform MỘT LẦN duy nhất
  SUBMIT zsf_in_178
